...
class Show extends Component {
  ...
  addCart = e => {
    ...
    if (this.props.cartItems.filter(item => item.itemId === this.state.itemsOnSale[idx]._id).length === 0) {
      this.props.setCartItems(arr)
    }

    // this.props.setCartItems({
    //   itemId: this.state.items[idx]._id,
    //   itemName: this.state.items[idx].name
    // })
  }
